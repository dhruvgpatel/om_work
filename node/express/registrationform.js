...
app.get("/", function (req, res) {
    ...
    con.query(`select option_name from job_application_db.option_master where select_id = 1;`, function (err, result1) {
        if (err) throw err;
        states = result1;
    });
    ...
    con.query(`select option_name from job_application_db.option_master where select_id = 2;`, function (err, result2) {
        if (err) throw err;
        status = result2;
        res.render('registrationform', { states, status, courses, departments, locations, languages, technologies });
...
app.post("/", (req, res) => {
    ...
    con.query(`select * from job_application_db.candidate_basic_info; `, function (err, result1) {
        if (err) throw err;
        var data = result1;
        res.render('registrationformsearch', { data: data });
...
app.get("/search", (req, res) => {
  var search = req.query.text
    ...
    con.query(sql, (err, result3) => {
        if (err) throw err;
        console.log(result3)
        res.render('registrationformsearch', { data: result3, multi })
    })
})